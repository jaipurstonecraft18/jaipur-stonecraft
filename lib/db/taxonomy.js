...
export async function getCollection(collectionSlug) {
  ...
  try {
    ...
    if (colRow) {
      const subRows = await query("SELECT * FROM subcategories WHERE parent_collection_slug = ? AND is_active = 1 ORDER BY sort_order ASC, id ASC", [collectionSlug]);
...
export async function getAllCollections() {
  ...
  try {
    ...
    for (const colRow of colRows) {
      const subRows = await query("SELECT * FROM subcategories WHERE parent_collection_slug = ? AND is_active = 1 ORDER BY sort_order ASC, id ASC", [colRow.slug]);
...
export async function getCategoriesBySubcategory(collectionSlug, subcategorySlug) {
  ...
  Object.values(categoriesData).forEach((cat) => {
    if (cat.parentCollection === collectionSlug && (!subcategorySlug || cat.parentSubcategory === subcategorySlug)) {
      mergedMap.set(cat.slug, { ...cat, sortOrder: cat.sortOrder ?? 0 });
    }
  });
  ...
  sortedList.sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0) || a.name.localeCompare(b.name));
  return sortedList;
}
...
export async function getAllCategories() {
  ...
  try {
    await initDB();
    const catRows = await query("SELECT * FROM categories WHERE is_active = 1 ORDER BY sort_order ASC, id ASC");
    for (const row of catRows) {
      const existingStatic = mergedMap.get(row.slug) || {};
      mergedMap.set(row.slug, {
        slug: row.slug,
