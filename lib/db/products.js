...
export async function formatProductFromRow(row) {
  ...
  const cleanCoverUrl = typeof rawCoverUrl === "string" 
    ? rawCoverUrl.trim().replace(/^["']|["']$/g, "") 
    : (rawCoverUrl?.src || rawCoverUrl?.url || "").trim().replace(/^["']|["']$/g, "");

  const imageSrc = (cleanCoverUrl && typeof cleanCoverUrl === "string" && cleanCoverUrl.trim() !== "") 
    ? cleanCoverUrl 
  ...
  const imageGallery = imageRows.map((img) => {
    const rawUrl = typeof img.url === "string" ? img.url : (img.url?.src || img.url?.url || "");
    const urlStr = typeof rawUrl === "string" ? rawUrl.trim().replace(/^["']|["']$/g, "") : "";
    return {
      src: urlStr,
      url: urlStr,
