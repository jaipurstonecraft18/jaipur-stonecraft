/**
 * Jaipur Stonecraft — Smart Catalogue Search Engine
 * 
 ...
 */
export function executeSmartSearch({ query, status, category, limit = 20 }) {
  if (!query || !query.trim()) {
    const db = getDB();
    ...
    return {
      query: "",
      products: rows.map(formatProductFromRow),
      categories: [],
      materials: [],
      subjects: [],
  ...
  // Fetch reference data from SQLite
  const allMaterials = db.prepare("SELECT * FROM materials WHERE is_active = 1").all();
  const allSubjects = db.prepare("SELECT * FROM subjects WHERE is_active = 1").all();
  const allCategories = db.prepare("SELECT * FROM categories WHERE is_active = 1").all();
  const allProductTypes = db.prepare("SELECT * FROM product_types WHERE is_active = 1").all();
  ...
  terms.forEach((term) => {
    ...
    // Check Materials with Typo Tolerance (Levenshtein distance <= 2)
  ...
  const allProductRows = db.prepare(sql).all(...params);
  ...
  if (matchedProducts.length === 0) {
    ...
    const fallbackRows = db.prepare("SELECT * FROM products WHERE status = 'published' ORDER BY is_featured DESC, updated_at DESC LIMIT 6").all();
    matchedProducts.push(...fallbackRows.map(formatProductFromRow));
    fallbackMessage = `No exact product found matching "${query}". Showing featured stonecraft creations below.`;
  }
