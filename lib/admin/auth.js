...
export function isAuthorizedSyncRequest(req) {
  try {
    ...
    const validSecret = process.env.MEDIA_SYNC_SECRET || process.env.ADMIN_SECRET_KEY || process.env.BACKUP_SECRET_KEY;
    if (!validSecret || !validSecret.trim()) return false;

    const expectedBuf = Buffer.from(validSecret.trim());
